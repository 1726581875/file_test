 ...
 * @date 2023/5/12
 */
public class DataChunkFileAccessor {
    ...
    public DataChunkFileAccessor(String fileFullPath) throws IOException {
    ...
    public DataChunkFileAccessor() throws IOException {
    ...
    private DataChunk writeRow(byte[] row, long startPos, Long rowId) {
        ...
        RowMetadata dataRow = new RowMetadata(dataChunk.getNextRowStartPos(), row, rid);
        // 当前块剩余空间足够，直接存储到该块
    ...
    public int writeRow(List<byte[]> rows) {
        ...
        for (int i = 0; i < rows.size(); i++) {
            byte[] row = rows.get(i);
            RowMetadata dataRow = new RowMetadata(dataChunk.getNextRowStartPos(), row, getNextRowId());
            // 当前块剩余空间足够，直接存储到该块
            if (dataChunk.remaining() >= dataRow.getTotalByteLen()) {
                dataChunk.addRow(dataRow);
            } else {
